...
public class UserServiceImplementation implements UserService {
  ...
  public UserDTO addUser(NewUserDTO newUser) {
    ...
    usersRepo.save(user);
    return converter.mapToUserDTO(user);
  }
  ...
  public List<UserDTO> getAllUsers() {
    ...
    for (User user : allUsers) {
      allUsersDTO.add(converter.mapToUserDTO(user));
    }
    return allUsersDTO;
  }
  ...
  public UserDTO updateUser(UserDTO user) {
    if (usersRepo.findById(user.getId()).isPresent()) {
      ...
      return converter.mapToUserDTO(userToUpdate);
    } else {
  ...
  public List<RoleDTO> getRolesForUser(Long userId) {
    ...
    for (Role role : roles) {
      rolesForUser.add(converter.mapToRoleDTO(role));
    }
    return rolesForUser;
  }
}
